...
import { type Address, createPublicClient, http, parseUnits, formatUnits } from 'viem';
...
const COINGECKO_BATCH_IDS = [
  ...
  'peppe', 'dogecoin', 'avalanche-2', 'matic-network',
...
async function fetchCoinGeckoPrices(): Promise<Record<string, number>> {
  try {
    const url = `${COINGECKO_API_BASE}/simple/price?ids=${COINGECKO_BATCH_IDS}&vs_currencies=usd&include_24hr_change=true`;
 ...
 * (symbol, name, price, 24h change). Powers the dynamic token picker —
 * no curated map to maintain.
 */
export async function fetchTopTokens(limit = 100): Promise<TopToken[]> {
  try {
    const url = `${COINGECKO_API_BASE}/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=${limit}&page=1&price_change_percentage=24h`;
    ...
    return rows
      ...
      .map((r) => ({
        ...
        name: r.name ?? (r.symbol as string).toUpperCase(),
...
export async function getTokenBalance(
  ...
  try {
    ...
    console.warn(`Failed to read ${tokenSymbol} balance:`, err);
    return 0;
  }
}
...
export async function getNativeBalance(
  ...
  try {
    ...
    console.warn('Failed to read native balance:', err);
    return 0;
  }
}
...
export async function getBlockNumber(chainId: number): Promise<number> {
  ...
  try {
    ...
    console.warn('Failed to read block number:', err);
    return 0;
  }
}
